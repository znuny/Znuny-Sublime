...
class ZnunyObjectDependencies(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        perl_module_check = re.search('.*[\/\\\\](.*?)\.pm$', file_path)  # noqa: W605
        # exit early if it is none
        if not perl_module_check:
            return
        ...
        for (dependency) in re.findall(pattern, body):
            ...
            if package_name and dependency == package_name:
                continue
            if dependency.find('$') != -1:
                continue
            if dependency in object_dependencies:
                continue
